...
app.post('/api/payment/shamcash', auth, async (req, res) => {
    try {
        ...
        dbData.subscriptions = dbData.subscriptions.filter(s => !(s.userId === req.user.id && s.status === 'pending'));
        db.writeDB(dbData);

        const payment = db.createPayment(req.user.id, amount, 'shamcash', reference);
        const sub = db.createSubscription(req.user.id, plan, amount, 'shamcash', 'pending');
...
app.post('/api/payment/confirm/:id', auth, adminOnly, async (req, res) => {
    try {
        ...
        const activated = db.activateSubscription(sub.id);
        console.log('✅ Subscription activated:', { subId: sub.id, userId: payment.userId, plan: sub.plan });
...
app.get('/api/admin/pending-payments', auth, adminOnly, (req, res) => {
    try {
        const dbData = db.readDB();
        const pending = dbData.payments.filter(p => p.status === 'pending');
        res.json({ 
            success: true, 
